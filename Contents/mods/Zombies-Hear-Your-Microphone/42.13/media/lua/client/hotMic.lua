...
function hotMic.onPlayerUpdate(playerObj)
    ...
    if traitsInfluence and traitsInfluence == 1 then
        ...
        traitsMultiplier = traitsMultiplier + ((traitsInfluence == 2 or traitsInfluence == 4) and playerObj:hasTrait(CharacterTrait.CONSPICUOUS) and 1 or 0)
        traitsMultiplier = traitsMultiplier + ((traitsInfluence == 2 or traitsInfluence == 4) and playerObj:hasTrait(CharacterTrait.CLUMSY) and 0.2 or 0)

        -- good traits 3 or 4 (both)
        traitsMultiplier = traitsMultiplier - ((traitsInfluence == 3 or traitsInfluence == 4) and playerObj:hasTrait(CharacterTrait.GRACEFUL) and 0.6 or 0)
        traitsMultiplier = traitsMultiplier - ((traitsInfluence == 3 or traitsInfluence == 4) and playerObj:hasTrait(CharacterTrait.INCONSPICUOUS) and 0.5 or 0)
